...
COMPONENTS = {
    ...
    "omega_consensus":        {"weight": 0.10, "path": HOME / "Omega-Production/omega_consensus.py"},
    ...
    "omega_node3":            {"weight": 0.05, "path": None},
    "omega_vps":              {"weight": 0.05, "path": None},
    "omega_cloud":            {"weight": 0.01, "path": None},
...
def score_component(name: str, info: dict) -> tuple[int, list]:
    ...
    if name == "omega_vps":
        try:
            ...
            try:
                ports = [v for v in data["port_assignments"].values()]
                if ports:
    ...
    if name == "omega_node3":
        try:
            ...
            if data.get("status") == "healthy":
                # Also check auth is enforced
                req2 = urllib.request.Request(
                    "http://127.0.0.1:5004/v1/storage/stats",
                    method="GET"
                )
                try:
                    urllib.request.urlopen(req2, timeout=3)
                    issues.append("Auth not enforced on storage endpoint")
                    return 50, issues
                except urllib.error.HTTPError as e:
                    if e.code == 401:
                        return 100, []
                    issues.append(f"Unexpected auth response: {e.code}")
                    return 75, issues
            issues.append("Node3 health check failed")
            return 50, issues
        except Exception as e:
            issues.append(f"Node3 unreachable: {e}")
            return 0, issues
