...
def check_button_freeze() -> bool:
    """Button contract — if it worked once, it must always work."""
    import json, re
    from pathlib import Path
    snap_path = Path("/data/data/com.termux/files/home/omega_button_snapshot.json")
    script_path = Path("/data/data/com.termux/files/home/omega_v10.py")
    
    if not snap_path.exists():
        _log("  ⚠️  No button snapshot — run snapshot first")
        return True
    
    snap = json.loads(snap_path.read_text())
    frozen_buttons = set(snap["buttons"].keys())
    
    src = script_path.read_text()
    current_buttons = set(re.findall(r'callback_data=["\'](.*?)["\']', src))
    
    # Check no buttons disappeared
    missing = frozen_buttons - current_buttons
    new_buttons = current_buttons - frozen_buttons
    
    all_ok = True
    if missing:
        _log(f"  ❌ BUTTONS REMOVED: {missing}")
        all_ok = False
    if new_buttons:
        _log(f"  ⚠️  NEW BUTTONS (need handler verification): {new_buttons}")
    if all_ok and not new_buttons:
        _log(f"  ✅ Button contract intact — {len(current_buttons)} buttons frozen")
...
def run_all():
    ...
    _log("")
    ok_buttons_freeze = check_button_freeze()
    ...
    _log("")
    _log("=" * 54)
    status = "✅ ALL CLEAR" if (ok_syntax and ok_buttons and ok_drift and ok_buttons_freeze) else "❌ ISSUES FOUND — see above"
    _log(f"  {status}")
    _log("=" * 54)
