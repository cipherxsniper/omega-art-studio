...
class Config:
    ...
    ONBOARDING_QUESTIONS = [ # streamlined for autonomous onboarding
...
def _main_menu(name: str) -> tuple[str, InlineKeyboardMarkup]:
    ...
    engine_btn = "⏸  Pause Engine" if not paused else "▶️  Resume Engine"
    text = (
        f"*OMEGA AI — MISSION CONTROL*\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"👤  Commander: {name}\n"
    ...
    kb = [
        [InlineKeyboardButton("── OMEGA AI BUSINESS ──", callback_data="menu")],
        [
        ...
        [InlineKeyboardButton("── OMEGA BANK ──", callback_data="menu")],
        [
            InlineKeyboardButton("🏛  Treasury",      callback_data="finance_treasury"),
            InlineKeyboardButton("🏦  Wallets",       callback_data="finance_bank"),
        ],
        [
            InlineKeyboardButton("📒  Ledger",        callback_data="ledger"),
            InlineKeyboardButton("💳  Cards",         callback_data="finance_cards"),
        ],
        [
            InlineKeyboardButton("💰  Revenue",       callback_data="revenue"),
            InlineKeyboardButton("🔍  Audit",         callback_data="finance_audit"),
        ],
        [InlineKeyboardButton("── OMEGA TRADING ──", callback_data="menu")],
        [
            InlineKeyboardButton("📈  Trade Desk",    callback_data="open_trading"),
            InlineKeyboardButton("📡  Signals",       callback_data="trade_signals"),
        ],
        [InlineKeyboardButton("── SYSTEM ──", callback_data="menu")],
        [
            InlineKeyboardButton("⚙️  System",        callback_data="system"),
            InlineKeyboardButton("📋  Logs",          callback_data="logs"),
        ],
        [InlineKeyboardButton(engine_btn, callback_data="toggle_engine")],
    ]
...
def send_daily_briefing_now(bot_token, admin_ids):
    ...
    try:
        ...
        try:
            ...
            _oracle = _sp.run(
                ["python3", "/data/data/com.termux/files/home/omega_oracle_v2.py"],
                capture_output=True, text=True, timeout=30
            )
            oracle_lines = [l for l in _oracle.stdout.split("\n") if "SCORE" in l or "Grade" in l or "ZERO ISSUES" in l or "Issues" in l]
            oracle_summary = "\n".join(oracle_lines[:4]) if oracle_lines else "Oracle: unavailable"
        except Exception as _oe:
            oracle_summary = f"Oracle error: {_oe}"
