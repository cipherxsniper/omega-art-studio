...
def _on_checkout_completed(data: dict):
    ...
    if email:
        ...
        # ── AUTO-PROVISION VPS NODE ──────────────────────────
        try:
            import sys as _sys
            _sys.path.insert(0, os.path.expanduser("~/omega_runtime"))
            from omega_vps_engine import provision, TIERS
            # Map product to VPS tier
            vps_tier_map = {
                "full_ops":     "sovereign",
                "growth_suite": "pro",
                "starter":      "lite",
            }
            vps_tier = vps_tier_map.get(product_key, "lite")
            creds = provision(
                owner_email       = email,
                owner_name        = name,
                tier              = vps_tier,
                stripe_payment_id = data.get("customer", ""),
            )
            # Send credentials via Telegram
            notify(
                f"🖥️ VPS PROVISIONED\n"
                f"  Client  : {name} <{email}>\n"
                f"  Tier    : {TIERS[vps_tier]['name']} (${TIERS[vps_tier]['price']}/mo)\n"
                f"  Instance: {creds['instance_id'][:8]}\n"
                f"  Port    : {creds['local_port']}\n"
                f"  API Key : {creds['api_key'][:20]}...\n"
                f"  Expires : {creds['expires_at'][:10]}"
            )
            # ── EMAIL CREDENTIALS TO CUSTOMER ─────────────────
            _tier = TIERS[vps_tier]
            _creds_body = f"""Hey {name},

Your Omega Cloud node is live.

━━━━━━━━━━━━━━━━━━━━━━━━━
YOUR NODE CREDENTIALS
━━━━━━━━━━━━━━━━━━━━━━━━━
Instance ID : {creds['instance_id'][:8]}
Tier        : {_tier['name']} (${_tier['price']}/mo)
Storage     : {_tier['storage_gb']}GB encrypted
Expires     : {creds['expires_at'][:10]}
...
━━━━━━━━━━━━━━━━━━━━━━━━━
Health      : GET  /health
Upload      : POST /v1/storage/upload
List        : GET  /v1/storage/list
Download    : GET  /v1/storage/object?object_id=X
Ledger      : POST /v1/ledger/event
Stats       : GET  /v1/storage/stats

━━━━━━━━━━━━━━━━━━━━━━━━━
QUICK TEST
━━━━━━━━━━━━━━━━━━━━━━━━━
curl https://omegaledgernode.netlify.app/health

━━━━━━━━━━━━━━━━━━━━━━━━━
YOUR TRUST ANCHOR
━━━━━━━━━━━━━━━━━━━━━━━━━
Your node is signed by the Omega Root CA.
Every transaction is SHA-256 hash-chained.
Your data is AES-256 encrypted.
No one — including us — can read your stored objects.
...
CEO, Omega AI"""
            ...
            Emailer.send(
                to          = email,
                subject     = f"Your Omega Cloud Node Is Live — {_tier['name']} Credentials",
                body        = _creds_body,
                product_key = product_key,
                add_sig     = False,
            )
            log("info", "vps", f"Credentials emailed to {email}")
        except Exception as _vps_err:
            notify(f"⚠️ VPS provisioning failed for {email}: {_vps_err}")
...
def _main_menu(name: str) -> tuple[str, InlineKeyboardMarkup]:
    ...
    kb = [
        ...
        [
            InlineKeyboardButton("☁️ Node 3",   callback_data="open_node3"),
        ],
        [
...
if TELEGRAM_OK:
    ...
    async def master_button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if data.startswith("finance_") or data == "open_finance":
            ...
            return
        # Route node3 callbacks
        if data.startswith("node3_") or data == "open_node3":
            await node3_button_handler(update, ctx)
            return
...
def build_telegram_app():
        ...
        app.add_handler(CommandHandler("node3", cmd_node3))
...
# ══════════════════════════════════════════════════════
# NODE 3 CLOUD — TELEGRAM DASHBOARD
# ══════════════════════════════════════════════════════

def _node3_menu():
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("💚 Health",      callback_data="node3_health"),
            InlineKeyboardButton("💾 Storage",     callback_data="node3_storage"),
        ],
        [
            InlineKeyboardButton("🖥️ Instances",   callback_data="node3_instances"),
            InlineKeyboardButton("🔑 Keys",        callback_data="node3_keys"),
        ],
        [
            InlineKeyboardButton("📊 Oracle",      callback_data="node3_oracle"),
            InlineKeyboardButton("📋 Ledger",      callback_data="node3_ledger"),
        ],
        [
...
async def cmd_node3(update, ctx):
    await _send_node3_dashboard(update, ctx)

async def _send_node3_dashboard(update, ctx):
    try:
        import urllib.request as _ur, json as _j, sys as _s, os as _o
        _s.path.insert(0, _o.path.expanduser("~/omega_runtime"))
        from omega_vps_engine import load_registry, status_all

        # Node 3 health
        try:
            res = _ur.urlopen("http://127.0.0.1:5004/health", timeout=3)
            h   = _j.loads(res.read())
            node3_status = "🟢 ONLINE"
        except:
            node3_status = "🔴 OFFLINE"

        # VPS stats
        s = status_all()

        # Oracle score
        score_file = _o.path.expanduser("~/omega_runtime/node3_score.json")
        node3_score = "N/A"
        if _o.path.exists(score_file):
            with open(score_file) as f:
                sd = _j.load(f)
            node3_score = f"{sd.get('score',0)}/100"

        # Storage stats
        try:
            from omega_storage import OmegaStorage
            stats = OmegaStorage.stats()
            storage_txt = f"{stats['total_objects']} objects / {stats['total_bytes']} bytes"
        except:
            storage_txt = "unavailable"

        sep = "=" * 32
        txt = (
            "OMEGA CLOUD - NODE 3\n"
            + f"{sep}\n"
            + f"Status    : {node3_status}\n"
            + f"Oracle    : {node3_score}\n"
            + f"Storage   : {storage_txt}\n"
            + f"{sep}\n"
            + "VPS ENGINE\n"
            + f"Active    : {s['active']} instance(s)\n"
            + f"Total     : {s['total_instances']} (incl. terminated)\n"
            + f"Next port : {s['next_port']}\n"
            + f"{sep}\n"
            + "ENDPOINTS\n"
            + "TLS  : port 5003\n"
            + "HTTP : port 5004\n"
            + "DDNS : omega-node3.duckdns.org\n"
            + "Web  : omegaledgernode.netlify.app"
        )
        kb = _node3_menu()
        ...
        err = f"Node 3 dashboard error: {e}"
        if update.callback_query:
            await update.callback_query.edit_message_text(err)
        else:
            await update.message.reply_text(err)

async def node3_button_handler(update, ctx):
    query = update.callback_query
    await query.answer()
    data  = query.data
    back  = InlineKeyboardMarkup([[
        InlineKeyboardButton("🔙 Node 3", callback_data="open_node3")
    ]])
    ...
    if data == "open_node3":
        ...
        try:
            ...
            txt = (
                ...
                f"Node   : {h.get('node','?')}\n"
                f"Time   : {h.get('time','?')[:19]}"
            )
        except Exception as e:
            txt = f"🔴 Node 3 unreachable: {e}"
        await query.edit_message_text(txt, reply_markup=back)

    elif data == "node3_storage":
        try:
            from omega_storage import OmegaStorage
            s   = OmegaStorage.stats()
            txt = (
                f"💾 NODE 3 STORAGE\n"
            ...
            txt = f"Storage error: {e}"
        await query.edit_message_text(txt, reply_markup=back)

    elif data == "node3_instances":
        try:
            from omega_vps_engine import load_registry
            reg      = load_registry()
            active   = [i for i in reg["instances"].values()
                       if i["status"] == "ACTIVE"]
            if not active:
                txt = "No active VPS instances."
            else:
                lines = ["🖥️ ACTIVE VPS INSTANCES"]
                for inst in active:
                    lines.append(
                        f"  {inst['instance_id'][:8]} | "
                        f"{inst['tier']:10} | "
                        f":{inst['port']} | "
                        f"{inst['owner_email'][:20]}"
                    )
                txt = "\n".join(lines)
        except Exception as e:
            txt = f"Instances error: {e}"
        await query.edit_message_text(txt, reply_markup=back)

    elif data == "node3_keys":
        try:
            from omega_auth import OmegaAuth
            keys = OmegaAuth.list_keys()
            active_keys = [k for k in keys if k.get("status") == "ACTIVE"]
            lines = [f"🔑 API KEYS ({len(active_keys)} active)"]
        ...
        try:
            ...
            if _o.path.exists(score_file):
                with open(score_file) as f:
                    sd = _j.load(f)
                lines = ["📊 NODE 3 ORACLE"]
                lines.append(f"Score     : {sd.get('score',0)}/100")
                lines.append(f"Best      : {sd.get('best_score',0)}/100")
                lines.append(f"Timestamp : {sd.get('timestamp','?')[:19]}")
                lines.append("")
                for name, comp in sd.get("components", {}).items():
                    icon = "✅" if comp.get("score",0) > 0 else "❌"
                    lines.append(f"  {icon} {name}: {comp.get('score',0)}/{comp.get('max',0)}")
                txt = "\n".join(lines)
            else:
                txt = "Oracle score file not found — run node3 guardian first"
        except Exception as e:
            txt = f"Oracle error: {e}"
        await query.edit_message_text(txt, reply_markup=back)

    elif data == "node3_ledger":
        try:
            ledger_path = _o.path.expanduser("~/omega_runtime/node3_ledger.jsonl")
            if _o.path.exists(ledger_path):
                with open(ledger_path) as f:
                    events = [_j.loads(l) for l in f if l.strip()]
                last = events[-5:] if len(events) >= 5 else events
                lines = [f"📋 NODE 3 LEDGER ({len(events)} events)"]
                ...
                txt = "No ledger events yet."
        except Exception as e:
            txt = f"Ledger error: {e}"
        await query.edit_message_text(txt, reply_markup=back)
...
async def cmd_companion(update, ctx):
    """Open Omega Companion — the AI that knows everything."""
    chat_id = str(update.effective_chat.id)
    if not _is_authed(chat_id):
        await update.message.reply_text("🔐 /start to authenticate")
        return
    text = (
        "🔮 *OMEGA COMPANION*\n\n"
        "Your personal AI — warm, perceptive, and she knows\n"
        "everything about the Omega system.\n\n"
        "📊 Oracle: 100/100\n"
        "🏦 Ledger: 2,005,913+ entries\n"
        "🌐 Node: omega-companion (Sovereign VPS)\n"
        "💾 Memory: stored in OmegaStorage\n\n"
        "Open in browser:\n"
        "`http://192.168.11.115:6010/companion`\n\n"
        "She knows the ledger, nodes, oracle, wallets,\n"
        "clients, OM109 — everything you built.\n\n"
        "_Talk to her about anything._"
    )
    from telegram import InlineKeyboardButton, InlineKeyboardMarkup
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("🔮 Open Companion", url="http://192.168.11.115:6010/companion"),
    ],[
        InlineKeyboardButton("📊 Status", callback_data="companion_status"),
        InlineKeyboardButton("🔙 Menu", callback_data="menu"),
    ]])
...
def singularity_dashboard():
    ...
    return (
        "\U0001f310 OMEGA SOVEREIGN SINGULARITY\n"
        "--------------------------------\n"
        "\U0001f9e0 OSI: ACTIVE (PoB VALIDATED)\n"
        "\U0001f310 REP: ACTIVE (1PB SCALING)\n"
        "\U0001f9ec OGI: ACTIVE (DNA ANCHORED)\n"
        "\U0001f512 LEDGER: OM109-SECURE\n"
        "\U0001f5a5 NODES: 2,530\n"
        "\U0001f4be CAPACITY: 250 TB\n"
        "--------------------------------"
    )

# Wire into the main command loop (example handler)
# if data == "/singularity":
#     bot.send_message(chat_id, singularity_dashboard())
# ------------------------------------
