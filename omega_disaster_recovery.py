...
def run_backup():
    ...
    remote_result = store_remote_copy(tarball, remote_host="192.168.11.2")
    if remote_result.get("ok"):
        log(f"Remote copy stored on 192.168.11.2: object_id={remote_result['object_id']} checksum={remote_result['checksum'][:16]}...")
    else:
