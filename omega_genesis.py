...
def capture_node_state(node_def):
    ...
    state = dict(node_def)
    state["captured_at"] = datetime.datetime.utcnow().isoformat() + "Z"
...
def capture_bank_state(conn_bank):
    ...
    try:
        ...
        state["wallet_count"] = cur.fetchone()[0]

        cur.execute("SELECT COUNT(*) FROM accounts")
        state["account_count"] = cur.fetchone()[0]

        cur.execute("""
            SELECT COALESCE(SUM(balance_cents),0)
            FROM wallets
        """)
        state["total_balance_cents"] = cur.fetchone()[0]

        cur.execute("SELECT COUNT(*) FROM virtual_cards")
        state["virtual_card_count"] = cur.fetchone()[0]
        ...
        """)
        row = cur.fetchone()
        state["treasury_reserve_cents"] = row[0] if row else 0
...
def bootstrap_genesis_tables(conn):
    """Create genesis tables if they don't exist."""
    try:
        cur = conn.cursor()
...
def write_quantum_genesis_event():
    ...
    log(f"  Date: {datetime.datetime.utcnow().isoformat()}Z")
    ...
    genesis_ts      = datetime.datetime.utcnow().isoformat() + "Z"
    ...
    if conn_ledger:
        try:
            if bootstrap_genesis_tables(conn_ledger):
                cur = conn_ledger.cursor()
                cur.execute("""
