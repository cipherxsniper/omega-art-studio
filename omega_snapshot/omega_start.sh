...
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no \
  ...
  "pkill -f omega_node_manager 2>/dev/null; sleep 1; \
   export OMEGA_NODE_ID=omega-node-002; \
   export OMEGA_NODE_HOST=192.168.11.163; \
   setsid python3 ~/Omega-Production/omega_bank/omega_node_manager.py \
   > ~/omega_runtime/logs/node_manager.log 2>&1 </dev/null & \
   sleep 3 && pgrep -f omega_node_manager > /dev/null && echo OK" \
