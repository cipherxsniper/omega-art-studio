path = "/data/data/com.termux/files/home/omega_marketplace.py"
with open(path) as f:
    src = f.read()

old = '''        # Log to ledger with UUID id
        entry_id = str(uuid.uuid4())
        cur.execute("""
            INSERT INTO ledger_entries
            (id, debit_account, credit_account, amount, memo, event_type, created_at)
            VALUES (%s, %s, %s, %s, %s, %s, NOW())
        """, (entry_id, from_wallet, to_wallet, amount, reason, 'TOKEN_TRANSFER'))'''

new = '''        # Log to ledger as TWO rows: a DEBIT and a CREDIT (true double-entry)
        debit_id = str(uuid.uuid4())
        credit_id = str(uuid.uuid4())
        cur.execute("""
            INSERT INTO ledger_entries
            (id, debit_account, credit_account, amount, memo, event_type, direction, created_at)
            VALUES (%s, %s, %s, %s, %s, %s, %s, NOW())
        """, (debit_id, from_wallet, to_wallet, amount, reason, 'TOKEN_TRANSFER', 'DEBIT'))
        cur.execute("""
            INSERT INTO ledger_entries
            (id, debit_account, credit_account, amount, memo, event_type, direction, created_at)
            VALUES (%s, %s, %s, %s, %s, %s, %s, NOW())
        """, (credit_id, from_wallet, to_wallet, amount, reason, 'TOKEN_TRANSFER', 'CREDIT'))'''

assert old in src, "PATCH FAILED: target block not found"
src = src.replace(old, new, 1)

with open(path, "w") as f:
    f.write(src)
print("PATCH APPLIED SUCCESSFULLY")
